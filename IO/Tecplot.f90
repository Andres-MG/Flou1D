...
!
!> @brief
!> Routines to save data to an ASCII tecplot (.dat) file.
!*******************************************************************************
...
module Tecplot
    ...
    public :: TecPlot_saveMesh
    public :: TecPlot_saveMap
...
!> @param[in]  outCoords    position of the nodes
...
!···············································································
subroutine TecPlot_saveMesh(meshUnit, baseName, outCoords, vars, time, &
                            interpolate)
    !* Arguments *!
    ...
    write(meshUnit, '(3a,'//tfmt//',a)') 'TITLE="', trim(baseName), '_', &
                                             time, '_solution"'
    write(meshUnit, '(3a)', advance="no") 'VARIABLES="', trim(vars(1)), '"'
    do i = 2, nvars
        write(meshUnit, '(3a)', advance="no") ' "', trim(vars(i)), '"'
    end do
    write(meshUnit, *) ''
    ...
    ! Get the output nodes is possible
    if (interpolate) then
        stdCoords = outCoords
    end if
    ...
    do i = 1, PDE%mesh%elems%size()
        ...
        write(meshUnit, '(a)') "ZONE"
        write(meshUnit, '(a, i0, a,'//tfmt//',3a, i0, a)') &
                                                'T="Element ',     elem%ID,   &
                                                '" SOLUTIONTIME=', time,      &
                                                ' ZONETYPE=',      'ORDERED', &
                                                ' I=',             std%n,     &
        ...
        ! Add spacing after each zone (just for readability)
        write(meshUnit, *) ''
...
!> @param[in]  time      time of the solution
!···············································································
subroutine TecPlot_saveMap(mapUnit, baseName, vars, time)
    !* Arguments *!
    ...
    ! Header of the file
    write(mapUnit, '(3a,'//tfmt//',a)') 'TITLE="', trim(baseName), '_', &
                                        time, '_map"'
    write(mapUnit, '(a)', advance="no") 'VARIABLES="App. order"'
    do i = 1, size(vars)
        write(mapUnit, '(3a)', advance="no") ' "', trim(vars(i)), '"'
    end do
    write(mapUnit, *) ''
    ...
    do i = 1, PDE%mesh%elems%size()
        ...
        ! Write header for the element
        write(mapUnit, '(a)') "ZONE"
        write(mapUnit, '(a, i0, a,'//tfmt//',3a, i0, a)') &
                                                'T="Element ',     elem%ID,  &
                                                '" SOLUTIONTIME=', time,     &
                                                ' ZONETYPE=','     ORDERED', &
                                                ' I=',             norders,  &
        ...
        do j = TruncError%MapLimit+1, ubound(elem%Terr, dim=1)
            write(mapUnit, fmat) j-1, (elem%Terr(j+1,k), k=1,NEQS)
        end do
...
end subroutine TecPlot_saveMap
